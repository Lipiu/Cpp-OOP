...
public:
    ...
    void displayWeather(){
        ...
        for(int i = 0; i < numberOfWeather; i++){
            cout << "Weather #" << (i+1) << " " << weatherData[i].getDate() << ", " << weatherData[i].getDescription() << ", ";
            if(weatherData[i].getLocation() != nullptr){
                cout << weatherData[i].getLocation() << endl;
            }
            else{
                cout << "Unknown location";
            }

            float* temps = weatherData[i].getTemperature();
            cout << "Temperatures: ";
            for(int j = 0; j < weatherData[i].getNumberOfElements(); j++){
                cout << temps[j] << " ";
            }
            cout << endl;
            cout << endl;
